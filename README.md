...
<p>このリポジトリは、catコマンドを実行してファイル最終行の末尾に改行があるかどうかで出力がどう変化するか確認します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、catコマンドを実行して出力された文字列を確認してみてください</p>
...
<div class="wp-block-group">
<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>git clone https://github.com/trygfmi/cat_last-new-line
cd cat_last-new-line
cat file1.txt file2.txt

cat file2.txt file1.txt</code></pre>
...
git clone https://github.com/trygfmi/cat_last-new-line
cd cat_last-new-line
cat file1.txt file2.txt
...
<div class="wp-block-group">
<p>file1.txtのファイル最終行の末尾に改行が無い状態で実行すると、出力結果のように後続の文字列がそのままの状態で文字列が連結されます<br>posix準拠のコマンドは行の末尾は改行で終わることを仕様として設計されているらしいので、なるべくファイル最終行の末尾に改行を追加して保存したいところです</p>
</div>
